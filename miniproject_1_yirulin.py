...
def load_glove_embeddings_gdrive(model_type):
    ...
    try:
        ...
        st.error(f"Error loading {model_type} files: {e}")
        st.warning("The downloaded files appear to be corrupted (likely a Google Drive download issue). Deleting them now. Please REFRESH the page to try downloading again.")
        if os.path.exists(word_index_temp):
            os.remove(word_index_temp)
        if os.path.exists(embeddings_temp):
            os.remove(embeddings_temp)
        st.stop()
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    count = 0
    for word in words:
        if word: # Handle empty strings from split
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        for index, category in enumerate(categories):
            cat_embedding = averaged_glove_embeddings_gdrive(category, word_index_dict, embeddings, model_type)
            cosine_sim[index] = cosine_similarity(input_embedding, cat_embedding)
        ...
        input_embedding = get_openai_embeddings(st.session_state.text_search, model_name=model_name)
        
        for index, category in enumerate(categories):
            if category in st.session_state[cache_key]:
                cat_embedding = st.session_state[cache_key][category]
                cosine_sim[index] = cosine_similarity(input_embedding, cat_embedding)
            else:
                cosine_sim[index] = 0.0
...
if __name__ == "__main__":
    ...
    if st.session_state.text_search:
        ...
        st.write("")
        st.write(
            "Demo developed by [yirulin](https://www.linkedin.com/in/your_id/ - Optional)"
        )
